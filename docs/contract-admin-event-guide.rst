...
This system is designed to simplify the administrative workload
...
#. Race services admin generates invoice document
#. FSRC Treasurer generates invoice from accounting system, including the invoice document
...
This describes the life cycle for contract race management.
   ...
   * Race Director agrees to contract electronically
     ...
     * Race director click's ACCEPT AGREEMENT button in email 
     * contract is put into 'confirmed' state
...
When the items in the contract have settled, the race services admin should initiate the invoice process. Many 
contracts will have all the items in the contract settled at the time the contract is agreed to by the race director,
...
When FSRC Admin Initiates Invoice (Treasurer)
-------------------------------------------------
When the race services admin initiates the invoice, the treasurer will receive
an email which contains a link to the invoice document generated in
:ref:`agreement-accepted-race-services`.

Using the accounting system, the treasurer should generate an invoice to the
:term:`client` as indicated by the financial policies. The invoice should
include either a link to the invoice document, or a downloaded PDF of the
invoice document. Once an invoice is generated, the treasurer should click the
**Invoice Sent** button on the :ref:`edit-event-view`. 
